    ...
    async def find_all_by_hotel_id(cls, hotel_id: int, date_from: date, date_to: date):
        async with async_session_maker() as session:
            ...
            query = select(
                ...
                cls.model.quantity,
                cls.model.image_id,
                (cls.model.price * func.extract('day', date_to - date_from)).label('total_cost'),
                (cls.model.quantity - func.coalesce(func.count(Bookings.id), 0)).label('rooms_left')  # Вычисление оставшихся комнат
            ).outerjoin(
                Bookings,
                and_(
